...
function get_url_status($url) {
	/*
	Sites Broken:
	http://centerforservice.byu.edu/ - 500
	http://www.sinolinkfoundation.org/ - 500
	http://www.guhsdaz.org/ -> HTML Page Redirect - Meta Redirect
	http://yjobs.byu.edu/ - https
	http://www.sorensongenomics.com/ - 200
	*/
	#$url = "http://www.bls.gov/oco/ocos048.htm#oes_links";
	$options = array(
		'return_info'	=> true,
		'return_body'   => false
	);
	...
	//print_r($result);
	return array('status' => $result['info']['http_code'], 'location' => $location);
	...
	*/
	...
	if(!$file_headers){
		$exists = false;
		return array('status' => 0, 'location' => $location);
	} elseif($file_headers[0] != 'HTTP/1.1 200 OK') {
		$exists = false;
		return array('status' => 0, 'location' => $location);
	} else {
		$exists = true;
		return array('status' => 200, 'location' => $location);
	}
...
*/
	...
	$url = parse_url($url);
	if(!isset($url['host'])){
		return array('status' => '500', 'location' => $location);
	}
	$host = $url['host'];
	$path = $url['path'];
	$port = 80;
	...
	$post .= "User-Agent: Web-sniffer/1.0.31\r\n";
	
	$post .= "\r\n";
	
	//Handles the E_WARNING
	//if(ping($host) === false){
	if(!host_exists($host)){
		print "\nURL: $incoming_url\nHost: $host\nThe above host doesn't exist according to the host_exist method\n";
		print "The File Headers:\n";
		print_r($file_headers);
		print "\n";
		return array('status' => 0, 'location' => $location);
	}
	$sock = fsockopen($host, $port, $errno, $errstr, 15.0);
	if($sock){
		fwrite($sock, $post, strlen($post));
		//gets the header for the current resource
		$header="";
		while (substr($header,-4,4)!="\r\n\r\n" && !feof($sock)){ 
			$header .= fgets($sock);
		}
		fclose($sock);
		...
		if(strpos($header, $locationFieldName)) {
			$startPosition = strpos($header, $locationFieldName) + strlen($locationFieldName);
			$endPosition = strpos($header, "\r\n",$startPosition) - $startPosition;
			$location = substr($header, $startPosition, $endPosition);
		}
	} else {
		print "\nURL: $incoming_url\nHost: $host\nThe address could not open a socket with that URL\n";
		print "The File Headers:\n";
		print_r($file_headers);
		print "\n";
	}
	print_r(array('url' => $incoming_url, 'status' => $status, 'location' => $location, 'exist' => $exists));
...
function send_report($email, $report, $from){
	...
	return mail($email, "Broken Links Report", $message, $from);

}
function host_exists($host){
	$socket  = socket_create(AF_INET, SOCK_RAW, 1);
	socket_set_option($socket, SOL_SOCKET, SO_RCVTIMEO, array('sec' => $timeout, 'usec' => 0));
    $successful_connection = socket_connect($socket, $host, null);
	socket_close($socket);
